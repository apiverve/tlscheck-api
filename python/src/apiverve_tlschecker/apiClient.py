...
class TlscheckAPIClientError(Exception):
...
class ValidationError(TlscheckAPIClientError):
...
class TlscheckAPIClient:
    # Validation rules for parameters (generated from schema)
    VALIDATION_RULES = {"domain": {"type": "string", "required": True}, "port": {"type": "integer", "required": False, "min": 1, "max": 65535, "default": 443}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the TlscheckAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises TlscheckAPIClientError: If API key is invalid
        """
        ...
        self.secure = secure
        self.debug = debug
        self.base_url = 'https://api.apiverve.com/v1/tlscheck'
        self.headers = {
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[TlscheckAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns TLSCheckerData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises TlscheckAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise TlscheckAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise TlscheckAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise TlscheckAPIClientError(f"Request failed: {str(e)}")
